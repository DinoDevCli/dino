...
```bash
dino --json scan leakage tests/dino/fixtures/scan/clean_code.py
dino --json scan leakage tests/dino/fixtures/scan/forbidden_import.py      # LEAKY_IMPORT
dino --json scan leakage tests/dino/fixtures/scan/shift_and_seedless.py    # SHIFT + SEEDLESS
dino --json scan leakage tests/dino/fixtures/scan/target_in_features.py    # TARGET_IN_FEATURES
...
dino --json map drift tests/dino/fixtures/map/repo_small \
...
.venv/bin/dino --json proof run --command echo proof_ok --repo dino/common \
  --scan tests/dino/fixtures/scan/clean_code.py --output-dir /tmp/dino_eval/proof_out
